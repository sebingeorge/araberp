...
//              
                string qry = @"select WorkShopRequestRefNo,WorkShopRequestDate,ItemRefNo,I.PartNo,ItemName,Quantity,UnitName from WorkShopRequestItem WI 
                                ...
                                WHERE W.isActive=1  AND W.WorkShopRequestDate BETWEEN  @from  AND @to  and I.ItemId = ISNULL(NULLIF(@itmid, 0), I.ItemId) AND  W.WorkShopRequestId = ISNULL(NULLIF(@id, 0), W.WorkShopRequestId) AND W.OrganizationId=@OrganizationId
